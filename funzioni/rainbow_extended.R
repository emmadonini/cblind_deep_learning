rainbow_extended <- function(im, cvd = c("protanopia", "deuteranopia", "tritanopia"), r = 1, g = 2, b = 3) {
 ...
 #Modello
 classifier <- load("classifier_RF_fin.RDS")
...
white <- white[with(white, order(R, G, B)),]
...
black <- black[with(black, order(R, G, B)),]

# Assemblare il dataset ordinato per colore per ricolorarlo con la palette viridis
fin <- rbind(black, purple, blue, lb, green, yellow, red, white)
